 ...
 *
 * This sets up:
 * - Automatic instrumentation for HTTP, Redis, PostgreSQL via auto-instrumentations
...
export function initializeTracing(): NodeSDK | null {
  ...
  const serviceName = process.env.OTEL_SERVICE_NAME || 'enrichment-service';
 ...
 *
 * @example
 * const result = await withSpan('enrichKillmail', async (span) => {
 *   span.setAttribute('killmail.id', killmailId);
 *   const enriched = await enricher.enrich(killmail);
 *   return enriched;
 * });
 */
